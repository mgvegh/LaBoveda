...
export default function CriptoPortfolioTracker() {
  ...
  useEffect(() => {
    ...
    const fetchPurchases = async () => {
      const { data, error } = await supabase.from('cripto_portfolio').select('*').eq('user_id', user.id);
      if (data && !error) {
  ...
  const handleAddPurchase = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.from('cripto_portfolio').insert([newRow]).select();
    ...
    if (data && !error) {
      ...
      alert("Error guardando en la Bóveda");
    }
  };

  const removePurchase = async (id: string) => {
    const { error } = await supabase.from('cripto_portfolio').delete().eq('id', id).eq('user_id', user?.id);
    if (!error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="glass-panel p-6 rounded-2xl md:col-span-1 border-teal-500/20 shadow-lg shadow-teal-500/5">
           ...
           <div className="text-3xl font-bold text-white mb-2">
             ${globalStats.totalValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
           </div>
        ...
        <div className="glass p-6 rounded-2xl md:col-span-1 flex flex-col justify-center border-teal-500/10">
           ...
           <div className="text-2xl font-bold text-gray-200">
             ${globalStats.totalInvested.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
           </div>
        </div>
                ...
                <div className="h-full w-1/3">
                  <ResponsiveContainer width="100%" height="100%">
                    <RePieChart>
                      ...
                      <ReTooltip formatter={(val: any) => `$${Number(val).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} />
                    </RePieChart>
                  </ResponsiveContainer>
                </div>
          ...
          <form onSubmit={handleAddPurchase} className="space-y-4">
            <div>
              ...
              <input 
                 ...
                 className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2.5 text-white focus:outline-none focus:border-teal-500 uppercase transition-colors"
              />
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2.5 text-white focus:outline-none focus:border-teal-500 transition-colors"
                />
              </div>
              <div>
        ...
        <div className="glass-panel p-6 rounded-2xl md:col-span-2 border-teal-500/10">
          <div className="flex items-center justify-between mb-6">
            ...
            <button 
              ...
              disabled={isLoading}
              className="p-2 text-teal-400 hover:text-teal-300 bg-teal-500/10 hover:bg-teal-500/20 rounded-lg transition-colors flex items-center gap-2 text-sm disabled:opacity-50"
            >
                  ...
                  {positions.map(p => (
                    <tr key={p.ticker} className="hover:bg-white/5 transition-colors group">
                      <td className="py-3 px-2">
                        <div className="font-bold text-gray-200">{p.ticker.replace('-USD', '')}</div>
                        <div className="text-xs text-gray-500 lg:hidden">${p.avgPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}/u</div>
                      </td>
                      ...
                      <td className="py-3 px-2 text-right">
                        {p.hasData ? (
                           <span className="font-medium text-teal-100">${p.currentPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        ) : (
                        ...
                        {p.hasData && (
                          <div className={clsx("flex flex-col items-end", p.pnlValue >= 0 ? "text-emerald-400" : "text-red-400")}>
                            ...
                            <div className="text-xs font-medium space-x-1">
                              <span className="opacity-70">${Math.abs(p.pnlValue).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
               ...
               <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
                 {purchases.map(p => (
                   <div key={p.id} className="flex justify-between items-center text-xs bg-black/20 p-2 rounded-md">
                     ...
                     <span className="font-bold text-teal-300 w-24">{p.ticker.replace('-USD', '')}</span>
